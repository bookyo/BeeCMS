    ...
    <template v-else>
      ...
      <v-col cols="12" md="12" class="mx-auto" v-if="files.length">
        <v-form ref="form" v-model="valid" lazy-validation>
          ...
          <div class="d-flex">
            <v-btn class="primary" :loading="loading" @click="addComic" :disabled="!valid"
              >提交</v-btn
            >
          </div>
        </v-form>
...
export default {
  ...
  methods: {
    async checkLimit() {
      ...
      const toadyComicCounts = getCounts.data._allComicsMeta.count;
      if (toadyComicCounts >= activeGroup.limitCreate) {
        this.limit = true;
      }
    },
    addFile: function () {
    ...
    async addComic() {
      ...
      if (this.$refs.form.validate()) {
        ...
        try {
          ...
          this.loading = false;
          this.$router.push("/comics/" + response.data.createComic.id);
        } catch (error) {
          console.log(error);
