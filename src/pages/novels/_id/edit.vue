  ...
  <v-row class="mx-auto">
    ...
    <v-col cols="12" md="12" class="mx-auto" v-if="files.length">
      <v-form ref="form" v-model="valid" lazy-validation>
        ...
        <div class="d-flex">
          <v-btn class="primary" @click="addNovel" :disabled="!valid"
            >提交</v-btn
          >
        </div>
      </v-form>
...
export default {
      ...
      oldFiles: [
        {
          ...
          source: novel.cover.file.publicUrl,
          serverId: novel.cover.id,
          ...
          options: {
            ...
            metadata: {
              poster: novel.cover.file.publicUrl,
            },
          },
        },
      ],
  ...
  methods: {
    ...
    async addNovel() {
      ...
      const isOld = file.source == this.novel.cover.file.publicUrl;
      if (
        file.source != this.novel.cover.file.publicUrl &&
        !this.finishUplaod
      ) {
      ...
      if (this.$refs.form.validate()) {
        ...
        try {
          ...
          this.$router.push("/novels/" + response.data.updateNovel.id);
        } catch (error) {
  ...
  head() {
    return {
        title: this.novel.title + ' - 编辑信息 - 蜂窝创作平台'
    };
  },
};
</script>
